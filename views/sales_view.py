...
class SalesView:
    ...
    def create_buttons_frame(self):
        ...
        delete_btn = tk.Button(manage_frame, text="QUITAR ARTICULO", width=15, borderwidth=3, 
                              bg=btnColor, fg='black', command=lambda: self.controller.delete_item() if self.controller else None)
        clear_btn = tk.Button(manage_frame, text="BORRAR TODO", width=15, borderwidth=3, 
                             bg=btnColor, fg='black', command=lambda: self.controller.clear_all() if self.controller else None)
        budget_btn = tk.Button(manage_frame, text="PRESUPUESTO", width=15, borderwidth=3, 
                               bg=btnColor, fg='black', command=lambda: self.controller.generate_budget() if self.controller else None)
        invoice_btn = tk.Button(manage_frame, text="FACTURA", width=15, borderwidth=3, 
